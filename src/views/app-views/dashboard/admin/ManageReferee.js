...
const ManageReferee = () => {
	...
	const onFinish = async (values) => {
		...
		const refereeAddress = values.refereeAddress
		const allowedMatches = values.allowedMatches

		const web3 = await walletServices(walletType)
		const OcoopData_contract =  new web3.eth.Contract(ACoopDataABI, ACoopDataAddress);
		await OcoopData_contract.methods.setUserAllowedMatches(refereeAddress,allowedMatches)
	...
	<>
	   <Row justify="center" gutter={16}>
		    <Col xs={24} sm={24} md={24} lg={12}>
		        <Card title='Managing Referee(심판 등록)' description='헬로?' >
					<Form layout="vertical" onFinish={onFinish} >
						<Form.Item name="refereeAddress" label="Referee's address(심판 지갑주소)" colon={false}
						rules={[
							  {
								required: true,
								message: 'Please input the Referee`s address!',
							  },
							]}
						>
						<Input />
						</Form.Item>
						<Form.Item name="allowedMatches" label="# of Matches(부여 경기수)" colon={false}
						rules={[
							  {
								required: true,
