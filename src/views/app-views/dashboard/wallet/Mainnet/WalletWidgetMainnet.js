...
const WalletWidgetOption = () => {
  ...
  <Menu>
    ...
    <Menu.Item key="1" onClick={ () => {}  }>
      <span>
        <div className="d-flex align-items-center">
			<a href='https://v2.info.uniswap.org/token/0xdeeb6091a5adc78fa0332bee5a38a8908b6b566e' target="_blank">
			  <SlidersOutlined />
...
const WalletWidgetMainnet = (props) => {
	...
	const [TACBalance, setTACBalance] = useState(null)
	...
	const getUserInfoFromFB = async() => {
	 ...
	 useEffect(()=>{	 
		 setTACBalance(tacLockedBalance + tacUnLockedBalance)
	 },[tacLockedBalance, tacUnLockedBalance])
			...
			<div className= 'd-flex mt-0'>
				<div>
						<div className="d-flex align-items-center">	
							...
							<span className = 'mb-0' style={{fontSize: '36px', fontWeight: '600', letterSpacing: '-1px' , color: '#15151f'}}>{(TACBalance !=null)? Math.floor(TACBalance) : 0}</span>
							...
							<span className = 'mb-0' style={{fontSize: '36px', fontWeight: '600', letterSpacing: '-1px' , color: '#d0d0d2'}}>
								{(TACBalance !=null)? Math.floor( (TACBalance-Math.floor(TACBalance))*1000  )  : 0}</span>				
							{(TACBalance !=null)?
							<span className = 'ml-3' style={{fontSize: '20px', fontWeight: '500', letterSpacing: '-0.4px', color: '#1cc760'}} >${TACBalance*tacPrice}</span>
								: <LoadingOutlined className = 'ml-3' style={{ fontSize: 24, color:'#1cc760'}} spin />
							} 	
						</div>
				  
						<Typography.Link  className="text-gray-light mt-0" 
							type="secondary" href={`https://etherscan.io/address/${walletAddress}`} target="_blank" copyable>
						 {walletAddress}
						</Typography.Link>	
...
export default WalletWidgetMainnet